  ...
  public async *runAgent(
    ...
    try {
      ...
      for await (const chunk of result) {
        // Find the node state in the chunk (it will be the first value)
        const nodeState = Object.values(chunk)[0] as any;
        if (nodeState && nodeState.messages && nodeState.messages.length > 0) {
          // Send the last message generated by this node
          const token = nodeState.messages[nodeState.messages.length - 1];
